...
public class Element implements WebElement, WrapsElement, WrapsDriver, org.openqa.selenium.interactions.Locatable {
		...
		Element(Driver driver, final WebElement element) {
			this.driver = driver;
		...
		public Element containsText(String text) {
		...
		public void isRestored() throws Exception {
			...
			while (x == false) {
				try {
					...
					Element newElement = driver.getElement(this.by);
		...
		public Element getElement(By by) {
			...
			while (x == false) {
				try {
					...
					if (++count > 200) {
						System.out.println(this.by.toString().substring(this.by.toString().indexOf(" ")+1) + by.toString().substring(by.toString().indexOf(" ")+2));
						x = true;
					}
				}
			}
			By lastBy = By.xpath(this.by.toString().substring(this.by.toString().indexOf(" ")+1) + by.toString().substring(by.toString().indexOf(" ")+2));
			this.driver.dispatcher.afterFindBy(by, element, this.driver.driver);
...
//		}
		...
		public void moveToView() throws Exception {
			...
			try {
				driver.manage().timeouts().implicitlyWait(500, TimeUnit.MILLISECONDS);
				Element navigationBar = this.findElement(By.xpath("//nav"));
